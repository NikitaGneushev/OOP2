class Student:
    ...
    def average_rate_hw(self, grades):
        self.average = sum(self.grades) / len(self.grades)
        return self.average

    def __str__(self):
        res = f'Имя: {self.name}, Фамилия: {self.surname}, Средняя оценка: {self.average}, Курсы в процессе изучения:{self.courses_in_progress}, Завершенные курсы: {self.finished_courses}'
        return res
...
class Lecturer(Mentor):
    ...
    def __str__(self):
        res = f'Имя: {self.name}, Фамилия: {self.surname}, Средняя оценка: {self.average}'
        return res
